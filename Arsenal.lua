...
if
    not pcall(
        function()
            readfile("Fu Kang Hub X//" .. Name)
        end
    )
 then
    writefile("Fu Kang Hub X//" .. Name, HttpService:JSONEncode(Des))
end

Settings = HttpService:JSONDecode(readfile("Fu Kang Hub X//" .. Name))

local function Save()
    writefile("Fu Kang Hub X//" .. Name, HttpService:JSONEncode(Settings))
end
